...
* The =.yas-setup= file
   Subdirectory could optionally contain a `.yes-setup', whose content will be evaluated as 
   elisp when the directory is loaded. It is a good place to put configuration code for one
   collection.
...
* The =.yas-skip= file
   Put .yas-skip file under the root would instruct yassnippet to skip loading that snippet table.
